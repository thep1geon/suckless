...
struct Client {
	...
	int bw, oldbw;
	unsigned int tags;
	int isfixed, isfloating, isurgent, neverfocus, oldstate, isfullscreen, orgtag;
	Client *next;
	Client *snext;
	Monitor *mon;
	Window win;
...
drawbar(Monitor *m)
{
	...
	for (i = 0; i < LENGTH(tags); i++) {
        ...
        int occupied = occ & 1 << i;

        schm = selected ? (i < LENGTH(colors) - 2 ? scheme[i] : scheme[SchemeSel]) : scheme[SchemeNorm];
        if (!show_all_tags) {
...
focus(Client *c)
{
	...
	if (c) {
		...
		XSetWindowBorder(dpy, c->win, scheme[i < LENGTH(colors)-2 ? i : SchemeSel][ColBorder].pixel);
		setfocus(c);
	} else {
...
run(void)
{
    ...
    for (i = 0; i < LENGTH(commands); ++i) {
        strcat(command_buffer, commands[i]);
